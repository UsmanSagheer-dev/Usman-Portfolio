...
const Header = () => {
  ...
  const navItems = [
    ...
    { id: "contact", label: "Free Consultation" },
  ];
  ...
  return (
    <>
      ...
      >
        <nav className="container mx-auto px-4 sm:px-6 py-3 sm:py-4">
          <div className="flex items-center justify-between">
            {/* Logo */}
            <div
              className="text-xl sm:text-2xl font-bold cursor-pointer hover:scale-105 transition-all duration-300"
            ...
            <div className="hidden md:flex items-center space-x-8">
              {navItems.map((item) => (
                ...
                >
                  {item.label}
                  <span
                    className={`absolute -bottom-1 left-0 w-full h-0.5 bg-gradient-to-r from-primary to-purple-500 rounded-full transition-all duration-300 ${
                      activeSection === item.id
              ...
              >
                {theme === "dark" ? (
                  <Sun
                    size={18}
                    className="text-yellow-500 group-hover:rotate-180 transition-transform duration-300"
                  />
                ) : (
                  <Moon
                    size={18}
                    className="text-blue-500 group-hover:rotate-180 transition-transform duration-300"
                  />
                )}
              </button>
            </div>
      ...
      >
        ...
        {/* Menu Content */}
        <div
          className={`absolute top-0 right-0 h-full w-72 sm:w-80 max-w-[85vw] sm:max-w-[80vw] bg-card/95 backdrop-blur-xl border-l border-border/50 shadow-2xl transform transition-transform duration-300 ${
